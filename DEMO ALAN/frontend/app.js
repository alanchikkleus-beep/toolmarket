...
/* ── Сравнение (до 3 позиций) ── */
const cmpItems = []; // [{query, category, market, compat}]
...
function cmpAdd(){
  ...
  cmpItems.push({
    ...
    compat: state.compatData,
  });
  updateCmpBadge();
  renderTab(); // обновить кнопку в результатах
}
...
function renderCmpContent(){
  ...
  const rows = [
    { key:"head",   render: (it,i) => `
      <div class="cmp-cell head">
        <div>
          <div style="font-size:.8rem;color:var(--muted)">${CAT_ICONS[it.category]||""} ${esc(it.category)}</div>
          <div>${esc(it.query)}</div>
        </div>
        <button class="cmp-add-btn" onclick="cmpRemove(${i})">✕</button>
      </div>` },
    { label:"Средняя цена", render: (it,i) => {
        const p = it.market?.stats?.avg_price;
        const best = p && p===minP;
        return `<div class="cmp-cell${best?" cmp-best":""}">
          <div class="cmp-price">${p ? p.toLocaleString("ru-RU")+" ₽" : "—"}</div>
          ${best?`<div style="font-size:.72rem;color:var(--success)">✅ Лучшая цена</div>`:""}
        </div>`;
    }},
    { label:"Диапазон цен", render: (it) => {
        const s = it.market?.stats||{};
        return `<div class="cmp-cell">${s.min_price!=null
          ? `от ${s.min_price.toLocaleString("ru-RU")} до ${s.max_price.toLocaleString("ru-RU")} ₽`
          : "—"}</div>`;
    }},
    { label:"Новый / Б.У.", render: (it) => {
        const s = it.market?.stats||{};
        return `<div class="cmp-cell">
          ${s.new_count>0 ? `<div style="color:var(--success)">✅ Новый: ${s.new_avg ? s.new_avg.toLocaleString("ru-RU")+" ₽" : "есть"} (${s.new_count} шт)</div>` : ""}
          ${s.used_count>0 ? `<div style="color:var(--warning)">🔄 Б/У: ${s.used_avg ? s.used_avg.toLocaleString("ru-RU")+" ₽" : "есть"} (${s.used_count} шт)</div>` : ""}
          ${!s.new_count && !s.used_count ? "—" : ""}
        </div>`;
    }},
    ...
    { label:"Применение", render: (it) => {
        const apps = it.compat?.applications||[];
        return `<div class="cmp-cell">${apps.length
          ? apps.map(a=>`<span class="app-tag" style="margin:2px;font-size:.73rem">${esc(a)}</span>`).join("")
          : "—"}</div>`;
    }},
    { label:"Отрасли", render: (it) => {
        const inds = it.compat?.industries||[];
        return `<div class="cmp-cell">${inds.length
          ? inds.map(a=>`<span class="app-tag" style="margin:2px;font-size:.73rem">🏭 ${esc(a)}</span>`).join("")
          : "—"}</div>`;
    }},
    { label:"Совм. бренды", render: (it) => {
        const brands = (it.compat?.compatible_holders_by_brand||[]).map(b=>b.brand);
        return `<div class="cmp-cell">${brands.length ? esc(brands.slice(0,4).join(", ")) : "—"}</div>`;
    }},
  ];

  // Build table as CSS grid
  let html = `<div class="cmp-grid" style="grid-template-columns:repeat(${cols},1fr)">`;

  rows.forEach(row => {
    if(row.label){
      // Full-width label row
      html += `<div class="cmp-cell label" style="grid-column:1/-1">${esc(row.label)}</div>`;
    }
  ...
  if(cmpItems.length < 3){
    html += `<div class="cmp-cell" style="grid-column:1/-1;text-align:center;padding:16px">
      <button class="btn btn-secondary" onclick="closeCompare()">
        + Добавить ещё позицию (найдите инструмент и нажмите «В сравнение»)
      </button>
    </div>`;
  }
...
function renderHistory(){
  ...
  const h = getHistory();
  if(!h.length){ el.innerHTML=`<span style="color:var(--muted);font-size:.8rem">История пуста</span>`; return; }
  el.innerHTML = h.map(q=>`
    <span class="chip" onclick="setQuery('${q.replace(/'/g,"\\'")}')">
      ${esc(q)}
    </span>
    `).join("")+
    `<span style="cursor:pointer;font-size:.75rem;color:var(--muted);padding:4px 8px;border-radius:20px;border:1px solid var(--border)"
      onclick="clearHistory()">✕ Очистить</span>`;
}
...
function loader(){ return `<div class="loader"><div class="spinner"></div>Загрузка...</div>`; }
...
function renderMarketEmpty(){
  const p = $("#tab-market");
  p.innerHTML = empty("🔍","Выберите категорию и введите название инструмента");
}
...
function renderMarket(){
  ...
  html += `<div style="margin-bottom:14px">
    ...
    ${cmpItems.length>1 ? `<button class="btn btn-secondary" style="margin-left:8px" onclick="openCompare()">Открыть сравнение (${cmpItems.length})</button>` : ""}
  </div>`;

  const q = encodeURIComponent(state.query);
  const avitoUrl = d.avito_url || `https://www.avito.ru/rossiya?q=${q}`;

  const markets = [
    { label:"🔍 Авито",           color:"#00aaff", text:"#fff", url:`https://www.avito.ru/rossiya?q=${q}` },
    { label:"🟡 Яндекс Маркет",  color:"#ffcc00", text:"#000", url:`https://market.yandex.ru/search?text=${q}&rs=eJwzKikpKDYEAAJ2AmM%3D` },
    ...
    { label:"🏭 Sandvik",         color:"#1a1a2e", text:"#ffd700", url:`https://www.sandvik.coromant.com/ru-ru/search#q=${q}` },
    { label:"💎 Iscar",           color:"#0057a8", text:"#fff", url:`https://www.iscar.com/eCatalog/item.aspx/lang/RU/Fnum/1?q=${q}` },
  ];

  html += `<div style="margin-bottom:6px;font-size:.75rem;color:var(--muted);text-transform:uppercase;letter-spacing:.05em">Найти на площадках</div>
  <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
    ${markets.map(m=>`<a href="${esc(m.url)}" target="_blank" rel="noopener"
      style="display:inline-flex;align-items:center;padding:7px 14px;background:${m.color};color:${m.text};border-radius:8px;text-decoration:none;font-weight:600;font-size:.8rem;white-space:nowrap">
      ${esc(m.label)}
    </a>`).join("")}
  </div>`;

  if(d.error && !d.listings?.length){
    html += `<div class="alert alert-warn">⚠ ${esc(d.error)} — показываем прямые ссылки на площадки выше.</div>`;
  }

  html += `
  <div class="section-header">
    <span class="section-title">
      ${CAT_ICONS[state.category]||""} "${esc(state.query)}"
    </span>
  ...
  <div class="stats-row">
    <div class="stat-card">
      ...
      ${s.min_price!=null?`<div style="font-size:.72rem;color:var(--muted)">Мин: ${fmt(s.min_price)} · Макс: ${fmt(s.max_price)}</div>`:""}
    </div>
    <div class="stat-card">
  ...
  if(d.listings?.length){
    ...
    d.listings.forEach(item => {
      ...
      const imgHtml = item.image
        ? `<img class="listing-img" src="${esc(item.image)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`
        : "";
    ...
    html += empty("📭","Объявлений не найдено. Попробуйте другой запрос.");
  }
...
function renderCompat(){
  ...
  if(state.loading){ p.innerHTML=loader(); return; }
  if(!state.compatData){
    p.innerHTML = empty("⚙️","Введите код пластины (например CNMG 120408) и нажмите Найти");
    return;
  }
  ...
  const pr = d.parsed||{};
  let html = `
    <div class="section-title" style="margin-bottom:10px">Пластина: <code>${esc(d.insert_code)}</code></div>
    <div class="parse-row">
  ...
  if(d.compatible_holders_by_brand?.length){
    ...
    d.compatible_holders_by_brand.forEach(b=>{
      html+=`<div class="brand-card">
        ...
        <div style="margin-bottom:6px"><div style="font-size:.71rem;color:var(--muted);margin-bottom:2px">Держатели:</div>
          ${b.holders.map(h=>`<span class="holder-tag" onclick="setQuery('${esc(h)}')">${esc(h)}</span>`).join("")}
        </div>
        <div><div style="font-size:.71rem;color:var(--muted);margin-bottom:2px">Пластины:</div>
...
function renderShapes(){
  ...
  let html = `<div class="section-title" style="margin-bottom:12px">Категории инструментов</div>
  ...
  html+=`<div class="section-title" style="margin-bottom:10px">Формы сменных пластин (ISO 1832)</div>
    <div class="shape-grid">`;
  Object.entries(shapes).forEach(([code,s])=>{
    html+=`<div class="shape-card" onclick="showShapeDetail('${code}')">
  ...
  html+=`<div class="section-title" style="margin:18px 0 10px">Группы материалов ISO 513</div>
    <div class="mat-grid">`;
  Object.entries(mats).forEach(([code,m])=>{
    html+=`<div class="mat-badge"><div class="mat-dot" style="background:${esc(m.color)}"></div><div><strong>${code}</strong> — ${esc(m.name)}<div style="font-size:.7rem;color:var(--muted)">${esc(m.description)}</div></div></div>`;
  });
  html+=`</div>`;
...
window.showShapeDetail = function(code){
  ...
  $("#shape-detail").innerHTML=`<div class="card" style="margin-top:10px">
    ...
    <div class="app-tags" style="margin-bottom:8px">${s.applications.map(a=>`<span class="app-tag">${esc(a)}</span>`).join("")}</div>
  </div>`;
};
...
function setupCalc(){
  ...
  function recalc(){
    ...
    if(p>0 && q>0){
      res.textContent = (p*q).toLocaleString("ru-RU")+" ₽";
    } else {
      res.textContent = "—";
    }
  }
